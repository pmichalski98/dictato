...
//!
//! **macOS only.** Both whisper-rs and llama-cpp-2 bundle ggml. On macOS the
//! linker resolves the shared `-lggml` to one archive (llama.cpp's, the
//! newer of the two) and Whisper was verified to run correctly on it. On
//! Windows and Linux the ggml objects are bundled into each rlib and the
//! link fails with duplicate symbols, so those builds get the stub below;
//! supporting them means running the LLM in a sidecar process.
//!
...
/// Stub for platforms where the in-process engine can't be linked; the
/// registry doesn't list LLMs there, so these are never reached in practice.
...
mod engine {
    ...
    const UNSUPPORTED: &str = "Local AI models are only available on macOS";

    #[derive(Clone, Default)]
    pub struct LocalLlmState;

    pub fn load_model(_state: &LocalLlmState, _model: LocalModel, _dir: &Path) -> Result<(), String> {
        Err(UNSUPPORTED.to_string())
    }

    pub fn unload_model(_state: &LocalLlmState) {}

    pub fn loaded_model(_state: &LocalLlmState) -> Option<LocalModel> {
        None
    }

    pub fn is_model_loaded(_state: &LocalLlmState, _model: LocalModel) -> bool {
        false
    }

    pub fn generate(_state: &LocalLlmState, _system: &str, _user: &str) -> Result<String, String> {
        Err(UNSUPPORTED.to_string())
    }
}
